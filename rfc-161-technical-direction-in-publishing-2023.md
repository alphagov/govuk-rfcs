...
manuals-publisher | Publishes manuals on GOV.UK | Retire in favour of content-publisher | Publishing | Short term: deliver quality of life improvementsLong term: consider whether app can be migrated into Whitehall, should Whitehall have support for hierarchical content structures.
...
signon | Single sign-on service for GOV.UK | No change expected | Publishing | Short term: invest in security and modernisation updates<br>Medium/long term: explore whether off-the-shelf software or third party integrations could replace this software or reduce it’s responsibilities
...
transition | Managing redirects for sites moving to GOV.UK. | Rename to "transition-admin" | Publishing | Medium term: merge destination for transition-config and bouncer. Simplify app removing functionality no longer used<br>Long term: replace app with text based config file (nginx or similar) for reduced maintenance given app usage is infrequent
...
- Is this need a variation on one our existing applications meet? If so, improve the existing app
...
Embrace relevant software engineering principles such as [separation of concerns](https://en.wikipedia.org/wiki/Separation_of_concerns), [single responsibility principle](https://en.wikipedia.org/wiki/Single-responsibility_principle), [You aren’t gonna need it (YAGNI)](https://en.wikipedia.org/wiki/You_aren't_gonna_need_it) and [The boy scout rule](https://www.oreilly.com/library/view/97-things-every/9780596809515/ch08.html). Embrace consistency, modularity and abstractions to manage complexity. Place value in [intuitive naming](https://medium.com/signal-v-noise/hunting-for-great-names-in-programming-16f624c8fc03).
...
If you’re part of a team that has inherited an incomplete migration, we encourage you to consider likelihood of the migrations success and whether it fits into future goals and seek to bring it to a close.
...
Avoid letting developer interventions blur the boundary of the service offered to users. Consider each time that a developer does something manually for a user as GOV.UK creating an undocumented feature that carries the expense of a developer’s time each usage. For frequent requests, that are not suitable as user features, document the boundaries of the service developers will provide and a runbook of how to achieve them.
...
Most of us started coding because it is fun and we want it to keep that as part of working on GOV.UK Publishing. 
