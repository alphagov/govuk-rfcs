...
Here we can see 13 TCP connections being opened (6 'credentialed', 6 'anonymous', 1 third-party to Google Analytics). If we weren't using SRI we can reduce this requirement down to 8 (6 'credentialed', 1 'anonymous' for fonts, 1 third-party to Google Analytics). This is the first step in inproving web performance for GOV.UK users on HTTP/1.1.
...
In the example test above on a Nexus 5 device under 3G connection speeds, we can bring the request of the CSS & JS files forwards by 750 ms. This should speed up the whole waterfall and turn the results from the summary list above from red to green in favour of HTTP/2.
...
The coalesced connection is under-utilised if 'anonymous mode' is used on our static assets. There's also the impact that [TCP Slow Start](https://en.wikipedia.org/wiki/TCP_congestion_control#Slow_start) has on the delayed `anonymous` connection, meaning assets will download slower than they could via a single coalesced connection. So we are in the following situation: the connection used to download the HTML isn't utilised fully (by this point it will already be up to speed), and the `anonymous` connection downloading the critical CSS has been delayed, so it isn't up to speed yet. For best performance we should be utilising the connection that has already been established via the HTML, to download other critical page assets (CSS/JS).
...
Basically the use of the wildcard (`*`) isn't allowed on a credentialed connection (`use-credentials`). And if it is used the browser will block any requests and post an error message to the that looks like this:
...
### 3 - The assets domain
Domain sharding for static assets is an anti-pattern under HTTP/2, and our current HTTP/1.1 setup isn't optimal for performance either. If our assets weren't being served with `Access-Control-Allow-Origin: *`, we would be able to change our `crossorigin` attribute from `anonymous` to `use-credentials`. This in turn would allow us to use correctly use [HTTP/2 connection coalescing](https://daniel.haxx.se/blog/2016/08/18/http2-connection-coalescing/), which would minimise the impact the assets domain is having on performance. This only applies for selected browsers, as some browser implementations are [notoriously flaky](https://bugs.chromium.org/p/chromium/issues/detail?id=1011685).

Users on older versions of Safari (before 12), IE, and Edge 18 and below all don't support coalescing, so won't benefit from this optimisation. But as we've seen above the domain sharding isn't offering up any benefits anyway, so why not remove the need for connection coalescing completely for static assets and serve them from the origin. All browsers that support HTTP/2 will then receive the same benefits and experience. [95.7% of the UK population](https://caniuse.com/#search=http2) use a browser that supports HTTP/2, so most of our users will benefit from this change.
...
The bulk of the page assets will be downloaded on the initial connection to the origin (www.gov.uk), with a secondary `anonymous` connection only opened for the fonts. That's twelve connections under our current HTTP/1.1 implementation, down to just two for HTTP/2 in many cases. So in order to fix our HTTP/1.1 and HTTP/2 performance issues we should serve static assets (CSS, JavaScript, fonts, images) all from the origin (www.gov.uk).

### Summary
In order to enable HTTP/2 on the GOV.UK domain (and for it to perform well) we need to consider problems 2 and 3 listed above:
...
- Switching to use `crossorigin='use-credentials'` isn't possible because we serve our assets with the `Access-Control-Allow-Origin: *` header so that the assets can be used on other domains (e.g. Heroku).
...
SRI Note: In serving the static assets from the origin domain, SRI no longer serves any purpose, as it's a security measure intended to be used on third party resources. As all of our assets are first party, it really isn't required and serves only to complicate our setup.
...
## Proposal
### MUST
* Continue to serve all our assets with `Access-Control-Allow-Origin: *` header. - Easy (no work required)
...
### Consequences
* Browsers that support HTTP/2 will be allowed to use it. Browsers that don't will still use HTTP/1.1.
